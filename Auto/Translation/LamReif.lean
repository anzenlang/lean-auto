...
section BuildChecker
  ...
  private def exprListToBinTree (l : List Expr) (lvl : Level) (ty : Expr) :=
    @instToExprBinTreeOfToLevel Expr (instExprToExprId ty) ⟨lvl, Prop⟩
    |>.toExpr (BinTree.ofListGet l)
  ...
  def compileRunResultExpr : ReifM Name := do
    ...
    mkNativeAuxDecl (Name.mkSimple "lam_ssrefl_rr") (Lean.mkConst ``RTable) runResultExpr
  ...
  def buildFullCheckerExprFor_indirectReduce_reflection (re : REntry) : ReifM Expr := do
    ...
    let u ← getU
    let lvtExpr := Lean.toExpr (BinTree.ofListGet ((← getVarVal).map Prod.snd).data)
    let lvtNativeName ← mkNativeAuxDecl (Name.mkSimple "lam_ssrefl_lvt") (Expr.app (.const ``BinTree [.zero]) (Lean.mkConst ``LamSort)) lvtExpr
    let litExpr := Lean.toExpr (BinTree.ofListGet (← getLamILTy).data)
    let litNativeName ← mkNativeAuxDecl (Name.mkSimple "lam_ssrefl_lit") (Expr.app (.const ``BinTree [.zero]) (Lean.mkConst ``LamSort)) litExpr
    let cpvExpr ← buildCPValExpr
    let cpvTy := Expr.const ``CPVal [u]
    let checker ← Meta.withLetDecl `cpval cpvTy cpvExpr fun cpvFVarExpr => do
      let (itExpr, ifExpr) ← buildImportTableExpr cpvFVarExpr
      let ifNativeName ← mkNativeAuxDecl (Name.mkSimple "lam_ssrefl_if") (Expr.app (.const ``BinTree [.zero]) (Lean.mkConst ``REntry)) ifExpr
      let csExpr ← buildChkStepsExpr
      let csNativeName ← mkNativeAuxDecl (Name.mkSimple "lam_ssrefl_cs") (Lean.mkConst ``ChkSteps) csExpr
      let .valid lctx t := re
        | throwError "buildFullCheckerExprFor :: {re} is not a `valid` entry"
      let vExpr := Lean.toExpr (← lookupREntryPos! re)
      ...
      let heqNativeName ← mkNativeAuxDecl (Name.mkSimple "lam_ssrefl_hEq") (Lean.mkConst ``Bool) heqBoolExpr
