...
: LamTerm.lamCheck?Eq' lval lctx (.bvar n) s := by
  dsimp [lamCheck?Eq', lamCheck?]; have ⟨hlt, _⟩ := List.getElem?_eq_some.mp h
  rw [pushLCtxs_lt hlt, List.getD_eq_getElem?_getD, h]
  rfl
...
theorem LamTerm.interpEq_bvar
  ...
  dsimp [interpEq, interp]; have ⟨hlt, _⟩ := List.getElem?_eq_some.mp h
