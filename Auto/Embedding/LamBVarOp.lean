...
theorem LamTerm.bvarLowersIdx?_bvar_eq_some :
  ...
  case true =>
    ...
    case false =>
      ...
      rw [← not_and]
      rw [← not_and]
      let to_not_or {p p' : Prop} (h : ¬ (p ∨ p')) : ¬ p ∧ ¬ p' :=
        not_or.mp h
      apply to_not_or ; clear to_not_or
      intro h ; cases h with
      | inl h => have h' := Nat.not_le_of_lt h.left; apply h' h₁'
      | inr h => have h' := Nat.not_le_of_lt h₂'; apply h' h.left
    case true =>
      have h₂' := Nat.le_of_ble_eq_true h₂; clear h₂
